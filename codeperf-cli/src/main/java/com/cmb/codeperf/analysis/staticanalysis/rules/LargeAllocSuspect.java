...
import com.cmb.codeperf.analysis.staticanalysis.ClassAnalysis.AllocSite;
        ...
        for (ClassAnalysis cls : classes) {
            for (MethodAnalysis m : cls.getMethods()) {
                for (AllocSite a : m.getAllocations()) {
                    boolean inLoop = m.inLoop(a.getInsnIdx());
                    String classMethod = cls.getClassName() + "." + m.getName();
                    String sizeStr = a.getSize() < 0 ? "变量尺寸" : (a.getSize() + " 字节");

                    if (inLoop && (a.getSize() < 0 || a.getSize() >= LOOP_BYTES_THRESHOLD)) {
                        findings.add(new StaticFinding(
                                TYPE, Severity.WARN, Confidence.MEDIUM,
                                "循环体内分配数组，可能造成高频内存分配与 GC 压力。",
                                "循环内数组分配（" + sizeStr + "，指令 #" + a.getInsnIdx() + "）。",
                                classMethod));
                    } else if (!inLoop && a.getSize() >= BIG_BYTES_THRESHOLD) {
                        findings.add(new StaticFinding(
                                TYPE, Severity.INFO, Confidence.LOW,
                                "方法中存在大数组分配（≥ 1MB），可能带来内存峰值。",
                                "大数组分配（" + sizeStr + "，指令 #" + a.getInsnIdx() + "）。",
                                classMethod));
                    }
                }
            }
