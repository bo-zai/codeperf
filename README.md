...
`codeperf init` 会自动生成 `.codeperf.yml`。生成逻辑会优先从 `remote.origin.url` 截取项目名，解析失败时回退到 Git 根目录文件夹名；但它不会理解你的 Maven/Gradle 模块结构、公司服务端地址或分支模型，真实项目接入后通常仍需要手工调整源码目录、服务端地址和 Git 基准分支。
...
```bash
java -javaagent:/opt/codeperf/codeperf-agent.jar=/etc/codeperf/agent.yml \
  -jar app.jar
```

`/etc/codeperf/agent.yml` 示例：
