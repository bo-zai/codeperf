...
`codeperf init` 会自动生成 `.codeperf.yml` 和 `.codeperf/agent.yml`。生成逻辑只负责提供可运行的默认模板，不会理解你的 Maven/Gradle 模块结构、公司服务端地址或业务包名；真实项目接入后通常需要手工调整项目名、源码目录、服务端地址和 agent 安装路径。
...
|---|---:|---:|---|
| `project` | 是 | 是 | 项目标识。`codeperf init` 默认生成 `demo`，真实项目建议改成仓库名或系统名，例如 `mall`、`music-education-app`。 |
