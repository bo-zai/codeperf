...
动态检测只作为测试/预发运行证据增强，不代表生产实测。企业接入推荐直接下载服务端返回的安装脚本，在 `docker build` 前自动完成接入，业务方不需要手工改 Dockerfile，也不需要自己拼 agent 版本、采样参数和目标包名。

```bash
CODEPERF_ENV=dev
curl -fsSL http://codeperf-server:9095/api/agent/install.sh | bash
```
